...
public class NotizTableModel  extends AbstractTableModel {
	...
	public String tablename = "Notiz";
	public Notiz searchObject = null;
	public String group = null;
	public Integer order = null;
	
	private final String [] columnName = {"Titel", "Note"};
	private final String [] columnDBName = {"n.TITLE", "n.NOTE"};
		...
		try{
			...
			if (searchObject!=null) {
				if (this.searchObject.getTitle()!=null){
					query +=" AND LOWER(n.TITLE) LIKE '%"+this.searchObject.getTitle().toLowerCase()+"%'";
				}
				if (this.searchObject.getNote()!=null){
					query +=" AND LOWER(n.Note) LIKE '%"+this.searchObject.getNote().toLowerCase()+"%'";
				}
	...
	public void readOnlineData(){
		try {
			...
			Notiz temp = (Notiz)this.ogdo.getObject(this.searchObject);
	 ...
	 * @param	Notiz	Das ist das Notiz das aus dem Datenbank entfernt werden soll.
	 * @return	boolean	Falls das Objekt erfolgreich gelöscht wurde dann wird true zurückgeliefert,
	 * sonnst false.
	 * @author	Alpay Firato
	 */
	public boolean deleteNotiz(Notiz Object){
		if (Object == null)
			return false;
		
		try{
			GenericDAO gdo = new GenericDAO();
			List <GenericDataObject> loeschen = gdo.unsafeQuery("SELECT * FROM " + this.tablename+" WHERE GLOBAL_ID ="+Object.getGlobalId(), new Notiz());
			lastDeletedObjects.add(loeschen.get(0));
			loeschen = gdo.unsafeQuery("DELETE FROM " + this.tablename+" WHERE GLOBAL_ID ="+Object.getGlobalId(), new Notiz());
			return true;
		} catch (Exception e){
			JOptionPane.showMessageDialog(null, "Error: " +e.toString(), "Error!" , JOptionPane.ERROR_MESSAGE);
		}
	...
	public void restoreLastDeletedObjects(){
		if (this.lastDeletedObjects.size() != 0) {
			for (int i=0;i<this.lastDeletedObjects.size();i++){
				addNotiz(this.lastDeletedObjects.get(i));
			}
	...
	public void downloadObject(){
		if (this.onlinesuche==true && this.objects != null && this.objects.size() != 0) {
			for (int i=0;i<this.objects.size();i++){
				addNotiz(this.objects.get(i));
			}
	 ...
	 * @author	Alpay Firato
	 */
	public void addNotiz(GenericDataObject Object){
		try{
			if (Object!=null){
	 ...
	 * @author	Alpay Firato
	 */
	public void setSearchObject (Notiz Object){
