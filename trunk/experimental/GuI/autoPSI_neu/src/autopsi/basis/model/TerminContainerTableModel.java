...
public class TerminContainerTableModel extends AbstractTableModel{
	...
	public String tablename = "TERMINCONTAINER";
	public TerminContainer searchObject = null;
	public String group = null;
	public Integer order = null;
	
	private final String [] columnName = {"Titel", "Beschreibung"};
	private final String [] columnDBName = {"t.Title", "t.DESCRIPTION"};
		...
		try{
			...
			if (searchObject!=null) {
				if (searchObject.getTitle()!=null) {
					query += " AND LOWER(t.TITLE) LIKE '%" + searchObject.getTitle().toLowerCase()+"%'";
				}
	...
	public void readOnlineData(){
		try {
			...
			TerminContainer temp = (TerminContainer)this.ogdo.getObject(this.searchObject);
	...
	public void deleteSelectedRow(JTable table) {
		TerminContainer p = new TerminContainer();
		...
		if (this.onlinesuche == false) {
		    for (int r=0; r<this.getRowCount(); r++) {
	            if (table.isCellSelected(r, 1)) {
	            	selected = true;
	            	p=(TerminContainer) this.getObjects().get(r);
	            	if (p.getId() != null ) {
	            		if (first) {
		            	...
		            	if (auswahl == JOptionPane.YES_OPTION) {
		            		//weiter mit löschen
		            		deleted = this.deleteTerminContainer(p);
		            		if (deleted) {
		            			if (first){
	 ...
	 * @param	TerminContainer	Das ist der TerminContainer der aus dem Datenbank entfernt werden soll.
	 * @return	boolean	Falls das Objekt erfolgreich gelöscht wurde dann wird true zurückgeliefert,
	 * sonnst false.
	 * @author	Alpay Firato
	 */
	public boolean deleteTerminContainer(TerminContainer Object){
		if (Object == null)
			return false;
		
		try{
			GenericDAO gdo = new GenericDAO();
			List <GenericDataObject> loeschen = gdo.unsafeQuery("SELECT * FROM " + this.tablename+" WHERE ID ="+Object.getId(), new TerminContainer());
			lastDeletedObjects.add(loeschen.get(0));
			loeschen = gdo.unsafeQuery("DELETE FROM " + this.tablename+" WHERE ID ="+Object.getId(), new TerminContainer());
			return true;
		} catch (Exception e){
			JOptionPane.showMessageDialog(null, "Error: " +e.toString(), "Error!" , JOptionPane.ERROR_MESSAGE);
		}
	...
	public void restoreLastDeletedObjects(){
		if (this.lastDeletedObjects.size() != 0) {
			for (int i=0;i<this.lastDeletedObjects.size();i++){
				addTerminContainer(this.lastDeletedObjects.get(i));
			}
	 ...
	 * @author	Alpay Firato
	 */
	public void addTerminContainer(GenericDataObject Object){
		try{
			if (Object!=null){
				...
				TerminContainer l = (TerminContainer) Object;
				l.setId(a.getId());
				l.setGroupID(0);
	 ...
	 * @author	Alpay Firato
	 */
	public void setSearchObject (TerminContainer Object){
	...
	public Object getValueAt(int row, int col) {
		TerminContainer ter = (TerminContainer) objects.get(row);
		if (ter==null)
			return null;
		else if (col==0)
			return ter.getTitle();
