...
public class mainFrame extends javax.swing.JFrame implements java.awt.event.MouseListener,java.awt.event.MouseMotionListener{
	...
	ArrayList<String> terminData = new ArrayList<String>();
	...
	public void mouseClicked(MouseEvent arg0) {
		...
		{ //zeigt den angeklickten Tag in der Infobar an
			...
			try
			{
				String[] value = (String[])table.getValueAt(table.rowAtPoint(table.getMousePosition()),table.columnAtPoint(table.getMousePosition()));
				c_marker.set(Integer.parseInt(value[0].substring(0,4)),Integer.parseInt(value[0].substring(5,7))-1,Integer.parseInt(value[0].substring(8,10)));
			...
			try
			{
				Object cellvalue = (table.getValueAt(table.rowAtPoint(table.getMousePosition()),table.columnAtPoint(table.getMousePosition())));
				if(cellvalue instanceof String[])
				{
				String [] data = (String[])cellvalue;
				
				Calendar cal2 = new GregorianCalendar();
	    		cal2.set(Integer.parseInt(data[0].substring(0,4)),Integer.parseInt(data[0].substring(5,7))-1,Integer.parseInt(data[0].substring(8,10)));
...
//		 TableZoom Engine (c) 2006 by Stephe ;-) ---------------------------
	...
	{
		String[] termine = new String[data.length-1];
		for(int i = 0;i<termine.length;i++)
			termine[i]=data[i+1];
		ListModel listmodel = new DefaultComboBoxModel(termine);
